function equivalentCheck() { //This allows the last value to remain in views until other buttons are pressed.
...
function input(x) {
    ...
    equivalentCheck();//#endregion

    let y = parseFloat(document.getElementById('result').value);

    if (document.getElementById('decimalVar').value == 0) //This checks if the decimal button was pressed
    {
    ...
    else //If decimal is true
    {
        ...
        if(decimalCount == 1)
        {
            x *= 1/10; //Using Maths to place the decimal in the correct place.
            y += x;
...
function decimalPoint() {
    if (document.getElementById('decimalVar').value == 0) //This prevents multiple decimal points from being added.
        document.getElementById('decimalVar').value = 1;

    if(parseInt(document.getElementById('operation').value)) //If this is an emptry string, it will return false.
    {
        document.getElementById('result').value = "0.";
    }
}
...
function operandCheck() {
    if(document.getElementById('operand').value == "") 
    {
        ...
        //document.getElementById('result').value = 0; //This is temporarily setting the result to zero so that the next number can be entered.
